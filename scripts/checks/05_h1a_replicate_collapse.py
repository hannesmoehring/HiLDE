...
for method in ("UMAP", "t-SNE", "PCA"):
    for label, thread_seed in (
        ("BEFORE B1 (seed unused)", False),
        ("AFTER  B1 (seed used) ", True),
    ):
...
print()
print(
    "UMAP varies after B1: the collapse is gone. MDS does too (not shown here; same check)."
)
print()
print(
    "PCA is identical in both arms by construction -- _pca takes no random_state and is"
)
print(
    "deterministic on these shapes, so its replicates never carried embedding variance."
)
print()
print(
    "t-SNE is identical in both arms for a DIFFERENT and less obvious reason: the seed IS"
)
print(
    "threaded now, but _tsne passes no `init`, and sklearn's default is init='pca', under"
)
print(
...
print(
    f"    sklearn TSNE, default init='pca'  -> max|diff| across random_state = {np.abs(_a - _b).max():.3e}"
)
print(
    f"    sklearn TSNE, init='random'       -> max|diff| across random_state = {np.abs(_c - _d).max():.3e}"
)
...
print()
print(
    f"  {'R':>3s}  {'collapsed: n=5R, R distinct':>30s}  {'fixed: n=5R, all distinct':>28s}  {'per-region: n=R':>20s}"
)
print("  " + "-" * 86)
...
for R in (8, 12, 20):
    ...
    print(
        f"  {R:3d}  n={collapsed.size:3d} p={p_collapsed:<20.7f}  n={fixed.size:3d} p={p_fixed:<18.7f}  n={per_region.size:3d} p={p_region:.7f}"
    )

print()
print(
    "Column 1 is the defect: R numbers repeated 5x, so n is inflated 5x and p shrinks"
)
