"""Streamlit-free dataset registry.

Extracted from the former `src/ui/data.py` so the FastAPI backend can load
datasets without importing Streamlit. Loaders are memoized with functools.cache
(the Streamlit UI used @st.cache_data). UI-only helpers (plot-df builders, CSV
export, interactive masks) were left behind with the Streamlit layer.
"""
