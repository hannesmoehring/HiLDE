...
SEED = 42  # seeds the global NumPy state (per-cell generators are seeded by the seed loop)
SEEDS = list(range(20))  # >=20 replicates for real CIs (design SS12.5); synthetic data is cheap
RHO_GRID = [1, 2, 4, 8, 16, 32]  # primary scale-separation sweep, the crossover knob (SS12.4)
NESTINGS = ["nested", "non_nested"]  # H2b control; non_nested is the negative control on ourselves
...
HIER_LAYERS = 2  # tree depth: layer 1 = coarse split, layer 2 = fine split (nested needs 2)
...
ROTATE_BLOCKS = True  # apply a global rotation Q so the planted blocks are not axis-aligned
MIN_CLUSTER_SIZE = 15  # HDBSCAN min_cluster_size, identical across ALL conditions (asserted)
...
RECOVERY_METRICS = ["h", "c", "v", "ari", "nmi", "within_g_ari", "coarse_ari", "noise_frac", "n_clusters", "frag_ratio"]
...
def make_nested_subspace(
    ...
    mu_a_group = base_sep * _unit_dirs(rng, g_groups, d_a)  # G coarse centroids (nested)
    mu_a_full = base_sep * _unit_dirs(rng, n_clusters, d_a)  # G*K centroids (non_nested)
    c_fine = base_sep * _unit_dirs(rng, k_sub, d_b)  # K base fine centroids, shared before rotation
    rot = [np.eye(d_b) if g == 0 else _rotation(rng, d_b, rotation_diversity) for g in range(g_groups)]
...
def within_group_hdbscan(block_b: np.ndarray, y_coarse: np.ndarray, cfg: Config) -> np.ndarray:
    ...
    for g in np.unique(y_coarse):
        ...
        labels = np.asarray(compute_clusters(block_b[idx], method="HDBSCAN", config=cfg)[0])
...
def mean_within_group_ari(part: np.ndarray, y_fine: np.ndarray, y_coarse: np.ndarray) -> float:
...
def score_condition(part: np.ndarray, y_fine: np.ndarray, y_coarse: np.ndarray, n_planted: int) -> dict:
    ...
    coarse = float("nan")
    if keep.sum() >= 2:
        coarse = float(adjusted_rand_score(y_coarse[keep], _to_modal(part, y_coarse)[keep]))
    return {
        "h": float(h),
        "c": float(c),
        "v": float(v),
...
def run_cell(rho: float, nesting: str, seed: int) -> tuple[list[dict], list[dict]]:
    ...
    parts = {
        ...
        "flat_full": np.asarray(compute_clusters(clustering_space(x_all, cfg), method="HDBSCAN", config=cfg)[0]),
        "flat_oracle_B": np.asarray(compute_clusters(block_b_std, method="HDBSCAN", config=cfg)[0]),
    ...
    rec = [{**base, "condition": name, **score_condition(part, y_fine, y_coarse, n_planted)} for name, part in parts.items()]
    ...
    with progress:
        ...
        if PARALLEL_JOBS == 1:
            for rho, nesting, seed in cells:
                progress.update(task, description=f"rho={rho} / {nesting} / s{seed}"[:48])
            ...
            jobs = (delayed(run_cell)(rho, nesting, seed) for rho, nesting, seed in cells)
        ...
        rows.append(
            {
                ...
                "wilcoxon_p": p,
                "rank_biserial": rbc,
                "oracle_relative": hier_mean / orc if orc and np.isfinite(orc) and orc != 0 else float("nan"),
            }
        )
...
def save_outputs(rec: pd.DataFrame, faith: pd.DataFrame, summary: pd.DataFrame, out_dir: Path) -> None:
...
def make_plots(rec: pd.DataFrame, out_dir: Path) -> None:
    ...
    if not nested.empty:
        ...
        sns.lineplot(data=nested, x="rho", y="within_g_ari", hue="condition", marker="o", errorbar=("ci", 95), ax=ax)
        ax.axhline(ADEQUACY, color="grey", lw=0.8, ls="--", label=f"adequacy={ADEQUACY}")
        ax.set_xscale("log", base=2)
        ax.set_xlabel("rho (coarse-vs-fine scale separation)")
        ax.set_ylabel("within-group ARI")
        ax.set_title("RQ1-S crossover: fine-structure recovery vs scale separation (nested)")
    ...
    if not nested.empty:
        ...
        sns.scatterplot(data=nested, x="h", y="c", hue="condition", style="condition", s=40, alpha=0.7, ax=ax)
        ax.plot([0, 1], [0, 1], color="grey", lw=0.6, ls=":")
        ax.set_xlabel("homogeneity (leaves pure)")
        ax.set_ylabel("completeness (cluster not split)")
        ax.set_title("Over-segmentation signature: high h, lower c => found but split (nested)")
        fig.tight_layout()
    ...
    if rec["rotation_diversity"].nunique() > 1:
        diag = rec[(rec["nesting"] == "nested") & (rec["condition"].isin(["hierarchical", "flat_oracle_B"]))]
        fig, ax = plt.subplots(figsize=(7, 5))
        sns.lineplot(data=diag, x="rotation_diversity", y="within_g_ari", hue="condition", marker="o", errorbar=("ci", 95), ax=ax)
        ...
        console.print("[dim]Skipped rotation diagnostic: single ROTATION_DIVERSITY (no sweep).[/]")
...
def print_summary(rec: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    if not nested.empty:
        table = Table(title="RQ1-S crossover (nested) - within-group ARI, delta = hierarchical - flat_full")
        for col in ["rho", "hier_mean", "flat_mean", "median_delta", "win_rate", "wilcoxon_p", "oracle_relative"]:
